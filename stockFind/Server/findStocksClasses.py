...
class Stock():
    ...
    def __init__(self, stock, dataFolder, flexible):
        ...
        self.processPath = 'C:\\Users\\ljlco\\PycharmProjects\\end\\stockFind\\Server\\Processed'
        self.s = stock
        ...
        self.daysClosed = [dt(2020, 1, 1), dt(2020, 1, 20), dt(2020, 2, 17),
                           ...
                           dt(2022, 1, 1), dt(2022, 1, 18), dt(2022, 2, 15),
                           dt(2022, 4, 2), dt(2022, 5, 31), dt(2022, 7, 5),
                           dt(2022, 9, 6), dt(2022, 11, 25), dt(2022, 12, 24)
                           ]
    ...
    def prelimTests(self):
        ...
        def removeExcess(b, i, rows, errorMessage='none'):
            """Remove excess rows."""
            # use minimum rows
            minbi = min(len(b), len(i))
            minRows = min(len(b), len(i)) if min(len(b), len(i)) < rows else rows
            b = b.iloc[:minRows, :]
            i = i.iloc[:minRows, :]
        ...
        # Remove excess number of rows                                  # currently 4 is an issue for certain stocks, as
        self.ab, self.ai, minRowsA, unaligna = removeExcess(self.ab, self.ai, 4) # they have data for less years, 2 works
