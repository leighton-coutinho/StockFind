...
def show_data(data):
    ...
    plt.savefig('C:\\Users\\ljlco\\PycharmProjects\\end\\stockFind\\static\\stockData.png')
    #plt.show()


def calculate_return(data):
    # NORMALIZATION - to measure all variables in comparable metric
    # shift gives us the previous value so that we can get daily returns, currdayval/prevdayval
    log_return = np.log(data / data.shift(1))
    # return first row forwards, as the first row will be undefined due to the shift
    return log_return[1:]
...
def show_mean_variance(returns, weights):
    ...
    # calculations using formulas from markowitz model
...
def show_portfolios(returns, volatilities):
    ...
    # use sharpe ratio for colors, returns/volatilities
...
def generate_portfolios(returns, stocks):
    ...
    # Use Monte Carlo simulations to generate portfolios
...
# scipy optimize module can only find the minimum of a given function
# hence we use the fact that the maximum of a f(x) is the minimum of -f(x) in order to find max sharpe ratio
...
def show_optimal_portfolio(opt, rets, portfolio_rets, portfolio_vols):
    ...
    plt.savefig("C:\\Users\\ljlco\\PycharmProjects\\end\\stockFind\\static\\optimalPortfolio.png")
    #plt.show()
